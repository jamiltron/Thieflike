...
-- if we do this we may as well define maxHeight and maxWidth,
-- but for now this reduces the number of arguments we are passing
-- around
minHeight = 6
minWidth  = 6
...
-- randomly generate connections inbetween rooms
...
expandRoom (Room _ ((x0,y0), (x1,y1)) _) = foldl fill M.empty coords
  where
    coords = concat [[(x,y) | x <- [x0-1..x1+1]] |
                              y <- [y0-1..y1+1]]
    fill lmap (x,y)
      | x >= x0 && x <= x1 && y >= y0 && y <= y1   = M.insert (x,y) '.' lmap
      | otherwise = M.insert (x,y) '#' lmap


-- given two coordinates, return a map where there is an open
-- path between the two coordinates
...
-- turn a coordinate into a hallway piece, this will open the given
-- coordinate and add hallways to blank adjacent squares
hallInsert :: Coord -> CharMap -> CharMap
hallInsert (x,y) m = let m' = foldl fill m (concat [[(x',y') | x' <- [x-1..x+1]]
                                                             | y' <- [y-1..y+1]])
                     in M.insert (x,y) '.' m'
  where
    fill m (x,y) = M.insertWith (\a b -> if b == ' ' then a else b) (x,y) '#' m
